...
let rec sl (i:int) : Tac unit =
  ...
  | Unknown (Some fv) ->
    ...
    ignore (repeat (fun () -> FStar.Tactics.split(); trefl `or_else` smt)); //definedness
    norm [];
    sl (i + 1)
...
// In my opinion, these lifts should be by elaboration, not by subtyping.
...
=
  let (x, y) = par (fun () -> r := 2; 1) (fun () -> l 2) in
  x + y
